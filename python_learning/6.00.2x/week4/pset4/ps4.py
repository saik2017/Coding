...
def evaluate_models_on_training(x, y, models):
    ...
    # TODO
    for model in models:
        estimate_values=np.polyval(model,x)
        r=round(r_squared(y,estimate_values),4)
        plt.plot(x,y,'bo',label='Actual values')
        plt.plot(x,estimate_values,'r-',label='Predicted values')
        plt.title('Approximation using a degree '+str(len(model)-1)+' polynomial to get r square:'+str(r))
        plt.xlabel("Year")
